...
impl<'a> ScenarioIntro<'a> {
    ...
    pub fn update_assemble(
        &self,
        actor: &RcRefCell<Character<'a>>,
        target: &RcRefCell<Character<'a>>,
    ) -> bool {
        let radius = target.borrow().get_collision()._bounding_box._mag_xz + 0.5;
        let (direction, dist) = math::make_normalize_xz_with_norm(
            &(target.borrow().get_position() - actor.borrow().get_position()),
        );
        if radius < dist {
    ...
    pub fn emoji_hungry(
        ...
        let contents = vec![TextBoxContent::MaterialInstance(String::from(
    ...
    pub fn create_move_to_tutorial_stage_text_box(
        &self,
        ...
        if let Some(prop) = self._prop_gate.as_ref() {
            ...
            game_scene_manager
                ...
                .add_text_box_item(TextBoxLayerType::GamePlayLayer, wrapper, &contents, None);
        }
    }

    pub fn remove_move_to_tutorial_stage_text_box(
        &self,
        game_scene_manager: &GameSceneManager<'a>,
    ) {
        if let Some(prop) = self._prop_gate.as_ref() {
            ...
            game_scene_manager
                .get_game_ui_manager_mut()
                .remove_text_box_item(wrapper.get_key());
        }
    }
    ...
    pub fn remove_hit_this_tree_text_box(&self, game_scene_manager: &GameSceneManager<'a>) {
        if let Some(prop_tree) = self._prop_tree.as_ref() {
            ...
            game_scene_manager
                .get_game_ui_manager_mut()
                .remove_text_box_item(actor_wrapper.get_key());
        }
    }
...
impl<'a> ScenarioBase<'a> for ScenarioIntro<'a> {
    ...
    fn on_open_game_scene(&mut self, game_scene_data_name: &str) {
        ...
        if game_scene_data_name == Stages::Home.get_stage_data_name() {
            ...
            self._prop_gate = Some(
                game_scene_manager
                    .get_prop_manager()
                    .get_prop_by_name(DEFAULT_GATE_NAME)
                    .unwrap()
                    .clone(),
            );
            ...
            self._prop_tree = Some(
                game_scene_manager
                    .get_prop_manager()
                    .get_prop_by_name("birch_tree_00")
                    .unwrap()
                    .clone(),
            );
            self._prop_gate_stage01 = Some(
                game_scene_manager
                    .get_prop_manager()
                    .get_prop_by_name(DEFAULT_GATE_NAME)
                    .unwrap()
        ...
        match self._scenario_track._scenario_phase {
            ScenarioPhase::None | ScenarioPhase::Begin => {
                ...
                self._around_end_position =
                    pivot - end_rotation_matrix.column(2).xyz() * CAMERA_DISTANCE_MIN;
                ...
                self._scenario_track
                    .set_next_scenario_phase(ScenarioPhase::StoryBoard, None);
            }
            ScenarioPhase::MoveToTutorialStage => {
                if game_scene_data_name == Stages::Home.get_stage_data_name() {
            ...
            ScenarioPhase::BackHome => {
                if game_scene_data_name == Stages::Home.get_stage_data_name() {
                    self._actor_ewa
                        ...
                        .unwrap()
                        .borrow_mut()
                        .set_hunger(HUNGER_WARNING_THRESHOLD);
                    self._actor_koa
                        .as_ref()
                        .unwrap()
                        .borrow_mut()
    ...
    fn update_game_scenario(
            ...
            match update_scenario_phase {
                ...
                ScenarioPhase::StoryBoard => match state {
                    State::Begin => {
                        ...
                        self._actor_aru
                            ...
                            .unwrap()
                            .borrow_mut()
                            .set_move_direction(&Vector3::new(1.0, 0.0, 0.0), true);
                        self._actor_ewa
                            .as_ref()
                            .unwrap()
                            .borrow_mut()
                    ...
                    State::Update => {
                        if SKIP_SCENARIO {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::MoveToTutorialStage, None);
                        } else {
                            let story_board_phase = self.get_story_board_phase();
                            if !USE_STORY_BOARDS || STORY_BOARDS.len() < story_board_phase {
                                self._scenario_track.set_next_scenario_phase(
                                    ScenarioPhase::Morning,
                                    Some(PHASE_TIME_SLEEP),
                                );
                            } else {
                ...
                ScenarioPhase::Morning => match state {
                    ...
                    State::Update => {
                        ...
                        if 1.0 <= phase_ratio {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::WakeUp, None);
                        }
                    }
                    _ => {}
                },
                ScenarioPhase::WakeUp => match state {
                    ...
                    State::Update => {
                        ...
                        if time_of_day_ratio < 1.0 {
                            game_scene_manager.set_time(
                                math::lerp(TIME_OF_DAWN, TIME_OF_EARLY_MORNING, time_of_day_ratio),
                                0.0,
                            );
                        }
                        ...
                        if self._wakeup_delay_koa < 0.0
                            ...
                            && self
                                ...
                                .unwrap()
                                .borrow_mut()
                                .is_action(ActionAnimationState::None)
                            && self
                                ._actor_ewa
                                .as_ref()
                                .unwrap()
                        ...
                        {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::AssembleFamily, None);
                        }
                    }
                    _ => {}
                },
                ScenarioPhase::AssembleFamily => {
                    if state == State::Update {
                        ...
                        if self.update_assemble(
                            self._actor_ewa.as_ref().unwrap(),
                            self._actor_aru.as_ref().unwrap(),
                        ) {
                ...
                ScenarioPhase::IamHungry => match state {
                    State::Begin => {
                        game_scene_manager
                            .get_scene_manager()
                            .play_audio_bank(AUDIO_STOMACH_GROWLING);

                        self._actor_aru
                            .as_ref()
                            .unwrap()
                            .borrow_mut()
                            .look_at(self._actor_koa.as_ref().unwrap().borrow().get_position());
                ...
                ScenarioPhase::MoveToTutorialStage => match state {
                    State::Begin => {
                        self._actor_ewa
                            ...
                            .unwrap()
                            .borrow_mut()
                            .set_next_behavior(BehaviorState::Idle, true);
                        self._actor_koa
                            .as_ref()
                            .unwrap()
                            .borrow_mut()
                        ...
                        let item_coconut = game_scene_manager
                            .get_game_resources()
                            .get_item_data(ITEM_COCONUT);
                        self._quest =
                            Some(game_ui_manager.add_quest(Some(String::from(
                                "Gather food for the hungry family.",
                            ))));
                        self._sub_quest_move_to_tutorial_stage =
                            Some(self._quest.as_ref().unwrap().borrow_mut().add_quest_item(
                                QuestCreateInfo::DefaultQuest(DefaultQuestData {
                                    _quest_icon_name: None,
                                    _quest_description: Some(String::from(
                                        "Move to the FOREST to find food.",
                                    )),
                                }),
                            ));
                        self._sub_quest_gather_food =
                            Some(self._quest.as_ref().unwrap().borrow_mut().add_quest_item(
                        ...
                        if SKIP_SCENARIO {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::WrapUpTheDay, None);
                        } else {
                            self.create_move_to_tutorial_stage_text_box(game_scene_manager);
                        }
                    }
                    State::Update => {
                        if game_scene_manager.get_current_game_scene_data_name()
                            == Stages::Forest.get_stage_data_name()
                        {
                            self._sub_quest_move_to_tutorial_stage
                                .as_ref()
                                .unwrap()
                                ...
                                .set_completed_quest();
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::GatheringFood, None);
                        }
                    }
                    _ => {}
                },
                ScenarioPhase::GatheringFood => match state {
                    ...
                    State::Update => {
                                ...
                                .is_completed_quest()
                        {
                            ...
                            self.create_return_home_text_box(game_scene_manager);
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::BackHome, None);
                        }
                    }
                    _ => {}
                },
                ScenarioPhase::BackHome => {
                    if state == State::Update
                        && game_scene_manager.get_current_game_scene_data_name()
                            == Stages::Home.get_stage_data_name()
                    {
                        self._sub_quest_back_home
                            .as_ref()
                            .unwrap()
                ...
                ScenarioPhase::Sleeping => {
                        ...
                        && !game_scene_manager.has_game_scenario(ScenarioType::ScenarioWrapUpTheDay)
                    {
                        self.clear_all();
                        game_scene_manager
                            .request_open_game_scenario(ScenarioType::ScenarioUfo, false);
                        self._scenario_track
        ...
        if self._sub_quest_sleep.is_some()
            && !self
                ...
                .borrow()
                .is_completed_quest()
            && let Some(scenario_wrap_up_the_day) = game_scene_manager
                .get_game_scenario(ScenarioType::ScenarioWrapUpTheDay)
                .as_ref()
        {
            ptr_as_mut((*scenario_wrap_up_the_day) as *const ScenarioWrapUpTheDay)
            ...
            self._sub_quest_sleep
                ...
                .set_completed_quest();
            self._scenario_track
                .set_next_scenario_phase(ScenarioPhase::Sleeping, None);
        }
    }
}
