...
impl<'a> ScenarioBase<'a> for ScenarioUfo<'a> {
    fn is_play_scenario_mode(&self) -> bool {
        match self._scenario_track._scenario_phase {
            ScenarioUfoPhase::MoveToTutorialStage |
            ScenarioUfoPhase::GatheringFood |
            ScenarioUfoPhase::BackHome |
            ScenarioUfoPhase::GiveFood |
            ScenarioUfoPhase::Sleep |
    ...
    fn update_game_scenario_begin(&mut self) {
        ...
        match self._scenario_track._scenario_phase {
            ...
            ScenarioUfoPhase::WakeUp => {
                game_scene_manager.get_scene_manager().play_audio_bank(AUDIO_ROOSTER);
            }
            ScenarioUfoPhase::AssembleFamily => {
            }
            ScenarioUfoPhase::IamHungry => {
    ...
    fn update_game_scenario(&mut self, game_ui_manager: &mut GameUIManager<'a>, any_key_hold: bool, any_key_pressed: bool, mut delta_time: f64) {
        ...
        match self._scenario_track._scenario_phase {
            ScenarioUfoPhase::None => {
                self.set_scenario_phase(ScenarioUfoPhase::StoryBoard.to_string().as_str(), None);
            }
            ScenarioUfoPhase::StoryBoard => {
                if SKIP_SCENARIO {
                    game_ui_manager.set_image_manual_fade_inout(MATERIAL_UI_NONE, INTRO_FADE_TIME);
                    game_ui_manager.set_auto_fade_inout(true);
                    self.set_scenario_phase(ScenarioUfoPhase::IamHungry.to_string().as_str(), Some(1.0));
                } else {
                    ...
                    if game_ui_manager.is_done_game_image_progress() && any_key_pressed {
                        if USE_STORY_BOARDS == false || STORY_BOARDS.len() <= story_board_phase {
                            game_ui_manager.set_image_manual_fade_inout(MATERIAL_UI_NONE, INTRO_FADE_TIME);
                            game_ui_manager.set_auto_fade_inout(true);
                            self.set_scenario_phase(ScenarioUfoPhase::Morning.to_string().as_str(), Some(PHASE_TIME_SLEEP));
                        } else {
            ...
            ScenarioUfoPhase::Morning => {
                ...
                if 1.0 <= phase_ratio {
                    self.set_scenario_phase(ScenarioUfoPhase::WakeUp.to_string().as_str(), None);
                }
            }
            ScenarioUfoPhase::WakeUp => {
                ...
                if self._wakeup_delay_koa < 0.0 &&
                    ...
                    self.set_scenario_phase(ScenarioUfoPhase::AssembleFamily.to_string().as_str(), None);
                }
            }
            ScenarioUfoPhase::AssembleFamily => {
                let mut done = true;
                ...
                if done {
                    self.set_scenario_phase(ScenarioUfoPhase::IamHungry.to_string().as_str(), Some(PHASE_TIME_HUNGRY));
                }
            }
            ScenarioUfoPhase::IamHungry => {
                if 1.0 <= phase_ratio {
                    self.set_scenario_phase(ScenarioUfoPhase::MoveToTutorialStage.to_string().as_str(), None);
                }
            }
            ScenarioUfoPhase::MoveToTutorialStage => {
                if game_scene_manager.get_current_game_scene_data_name() == Stages::Forest.get_stage_data_name() {
                    ...
                    self.set_scenario_phase(ScenarioUfoPhase::GatheringFood.to_string().as_str(), None);
                }
            }
            ScenarioUfoPhase::GatheringFood => {
                if game_scene_manager.get_current_game_scene_data_name() == Stages::Forest.get_stage_data_name() {
                    if self._sub_quest_gather_food.as_ref().unwrap().borrow().is_completed_quest() {
                        ...
                        self.remove_hit_this_tree_text_box(game_scene_manager);
                        self.create_return_home_text_box(game_scene_manager);
                        self.set_scenario_phase(ScenarioUfoPhase::BackHome.to_string().as_str(), None);
                    }
                }
            }
            ScenarioUfoPhase::BackHome => {
                if game_scene_manager.get_current_game_scene_data_name() == Stages::Home.get_stage_data_name() {
                    ...
                    self.remove_return_home_text_box(game_scene_manager);
                    self.set_scenario_phase(ScenarioUfoPhase::GiveFood.to_string().as_str(), None);
                }
            }
            ScenarioUfoPhase::GiveFood => {
                if game_scene_manager.get_current_game_scene_data_name() == Stages::Home.get_stage_data_name() {
                    ...
                    if sub_quest_give_food_to_ewa && sub_quest_give_food_to_koa {
                        self.set_scenario_phase(ScenarioUfoPhase::Sleep.to_string().as_str(), None);
                    }
                }
            }
            ScenarioUfoPhase::Sleep => {
                if game_scene_manager.get_current_game_scene_data_name() == Stages::Home.get_stage_data_name() {
                    ...
                    if let Some(quest) = &self._quest {
                        if quest.borrow().is_completed_quest() {
                            self.set_scenario_phase(ScenarioUfoPhase::End.to_string().as_str(), None);
                        }
                    };
                }
            }
