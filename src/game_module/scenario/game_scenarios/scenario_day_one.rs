...
impl<'a> ScenarioDayOne<'a> {
    ...
    pub fn update_release_family(&mut self, delta_time: f64) -> bool {
        let arrived_aru = self.update_release_actor(
            self._actor_aru.as_ref().unwrap().clone(),
            self._prop_bed_for_aru.as_ref().unwrap().clone(),
            delta_time,
        );
        let arrived_ewa = self.update_release_actor(
            self._actor_ewa.as_ref().unwrap().clone(),
            self._prop_bed_for_ewa.as_ref().unwrap().clone(),
            delta_time,
        );
        let arrived_koa = self.update_release_actor(
            self._actor_koa.as_ref().unwrap().clone(),
            self._prop_bed_for_koa.as_ref().unwrap().clone(),
            delta_time,
        );
    ...
    pub fn drop_monolith(&mut self, delta_time: f64) -> bool {
        ...
        let moved_pos = pos_before + math::safe_normalize(&to_target) * move_dist;
        let range = 0.1f32;
        if math::check_arrival_with_radius(
            &pos_before,
            &moved_pos,
            ...
            match update_scenario_phase {
                ...
                ScenarioPhase::Begin => {
                    if state == State::Update {
                        if unsafe { SKIP_SCENARIO } {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::Awake, Some(6.0));
                        } else {
                            game_scene_manager.set_time(TIME_OF_EARLY_MORNING, 0.0);
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::ReleaseFamily, Some(6.0));
                        }
                    }
                }
                ScenarioPhase::ReleaseFamily => match state {
                    ...
                    State::Update => {
                        ...
                        if complete {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::UfoGone, Some(3.0));
                        }
                    }
                    _ => {}
                },
                ScenarioPhase::UfoGone => {
                    if state == State::Update {
                        self._actor_ufo
                            ...
                            .unwrap()
                            .borrow_mut()
                            .set_move(&Vector3::new(0.0, 0.0, -1.0));
                        if 1.0 <= phase_ratio {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::DropMonolith, None);
                        }
                    }
                }
                ScenarioPhase::DropMonolith => {
                    if state == State::Update {
                        ...
                        if self
                            ...
                            .unwrap()
                            .borrow_mut()
                            .move_to_target(&self._monolith_start_position, radius)
                        {
                            if !self
                                ._prop_monolith
                                .as_ref()
                        ...
                        if drop_completed {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::ReallyUfoGone, Some(3.0));
                        }
                    }
                }
                ScenarioPhase::ReallyUfoGone => match state {
                    State::Update => {
                        ...
                        if 1.0 <= phase_ratio {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::CloseUpShot, Some(3.0));
                        }
                    }
                    State::End => {
                ...
                ScenarioPhase::CloseUpShot => match state {
                    ...
                    State::Update => {
                        if 1.0 <= phase_ratio {
                            self._scenario_track
                                .set_next_scenario_phase(ScenarioPhase::Awake, Some(5.0));
                        }
                    }
                    _ => {}
                },
                ScenarioPhase::Awake => match state {
                    State::Begin => {
                        game_scene_manager
                            ...
                            .remove_character(self._actor_ufo.as_ref().unwrap());
