...
impl<'a> KeyBindingWidget<'a> {
    pub fn update_icon_material_instance(&mut self, is_keyboard_input_mode: bool) {
        let binding_icons = if is_keyboard_input_mode { &self._key_binding_icons } else { &self._joystick_binding_icons };

        ptr_as_mut(self._layout_widget)._ui_component.set_visible(binding_icons.is_empty() == false);
        ...
        for (index, icon_widget) in self._binding_icon_widgets.iter().enumerate() {
            if index < binding_icons.len() {
                ...
                ptr_as_mut(*icon_widget)._ui_component.set_material_instance(
                    binding_icons.get(index).map(|material_instance_data| material_instance_data.clone())
                );
            } else {
...
impl Default for KeyBindingWidgetMap<'_> {
    fn default() -> Self {
        KeyBindingWidgetMap {
            _key_binding_widget_map: HashMap::new()
        }
    }
}
