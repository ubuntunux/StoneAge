use crate::game_module::widgets::game_menu_widget::character_list_helper::collect_all_characters;
...
impl<'a> TamingListWidget<'a> {
    ...
    pub fn refresh_taming_list(&mut self) {
        ...
        let mut tamed_list: Vec<_> = all_characters.into_iter().filter(|c| c.is_tamed).collect();

        tamed_list.sort_by(|a, b| {
            b.is_alive
                .cmp(&a.is_alive)
                .then_with(|| b.intimacy.partial_cmp(&a.intimacy).unwrap_or(std::cmp::Ordering::Equal))
                .then_with(|| a.name.cmp(&b.name))
        });

        let tamed_count = tamed_list.len();

        for item in tamed_list.iter() {
