...
impl<'a> CraftWidget<'a> {
    ...
    pub fn create_craft_widget(parent_widget: &mut WidgetDefault<'a>) -> CraftWidget<'a> {
        ...
        for (idx, recipe) in CRAFT_RECIPES.iter().enumerate() {
            ...
            let product_set =
                UIManager::create_widget(&format!("craft_prod_set_{}", recipe.id), UIWidgetTypes::Default);
            ...
            let product_hdr =
                UIManager::create_widget(&format!("craft_prod_hdr_{}", recipe.id), UIWidgetTypes::Default);
            ...
            for (ing_idx, req) in recipe.materials.iter().enumerate() {
                let ing_set = UIManager::create_widget(
                    &format!("craft_ing_set_{}_{}", recipe.id, ing_idx),
                ...
                let ing_icon = UIManager::create_widget(
                    &format!("craft_ing_icon_{}_{}", recipe.id, ing_idx),
                    UIWidgetTypes::Default,
                );
                ...
                let ing_lbl = UIManager::create_widget(
                    &format!("craft_ing_lbl_{}_{}", recipe.id, ing_idx),
                    UIWidgetTypes::Default,
                );
