...
impl<'a> Default for BehaviorDefault<'a> {
    fn default() -> Self {
        Self {
            ...
            match update_behavior_state {
                BehaviorState::Idle => {
                    match state {
                        State::Begin => {
                            owner.set_move_idle();
                            self._behavior_data.set_behavior_time(lerp(NPC_IDLE_TERM_MIN, NPC_IDLE_TERM_MAX, rand::random::<f32>()));
                        }
                        State::Update => {
                ...
                BehaviorState::Roaming => {
                    match state {
                        State::Begin => {
                            ...
                            self._behavior_data._move_direction = math::safe_normalize(&(self._behavior_data._target_point - owner.get_position()));
