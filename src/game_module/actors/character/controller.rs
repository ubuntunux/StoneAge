...
impl<'a> CharacterController<'a> {
    ...
    pub fn update_fly_controller(
        ...
        if move_direction.x != 0.0 || move_direction.z != 0.0 {
            ...
            self._velocity.x = move_direction.x * move_speed;
            self._velocity.z = move_direction.z * move_speed;
        } else {
        ...
        {
            let delta = self._position - self._prev_position;
            if MOVE_LIMIT < delta.x.abs() {
                self._position.x = self._prev_position.x + delta.x.signum() * MOVE_LIMIT;
            }

            if MOVE_LIMIT < delta.y.abs() {
                self._position.y = self._prev_position.y + delta.y.signum() * MOVE_LIMIT;
            }

            if MOVE_LIMIT < delta.z.abs() {
                self._position.z = self._prev_position.z + delta.z.signum() * MOVE_LIMIT;
            }
        }
    ...
    pub fn update_move_speed(&mut self, delta_time: f32) -> f32 {
        if self._is_jump {
            return self._final_move_speed * self._running_multiplier;
        }
        ...
        self._final_move_speed * self._running_multiplier
    }
    ...
    pub fn update_ground_controller(
        ...
        if move_direction.x != 0.0 || move_direction.z != 0.0 {
            ...
            self._velocity.x = move_direction.x * move_speed * air_speed_factor;
            self._velocity.z = move_direction.z * move_speed * air_speed_factor;
        } else {
            self._velocity.x = 0.0;
            self._velocity.z = 0.0;
        }
        ...
        begin_block!("apply slop velocity");
        {
            ...
            if self.is_on_ground() {
                ...
                if SLOPE_ANGLE <= ground_normal_y {
                    let slope_decay =
                        SLOPE_VELOCITY_DECAY * (ground_normal_y - SLOPE_VELOCITY_DECAY) / (1.0 - SLOPE_VELOCITY_DECAY);
        ...
        let mut move_delta = self._position - self._prev_position;
        ...
        let collision_pos_min = math::get_min(
            &actor_collision._bounding_box._min,
            &current_actor_collision._bounding_box._min,
        );
        let collision_pos_max = math::get_max(
                ...
                for recheck_collision_object in collision_objects.values() {
                    ...
                    if collided_block != recheck_block {
                        ...
                        if current_actor_collision.collide_collision(&recheck_block._collision) {
                            if self._velocity.y <= 0.0 && recheck_block_bound_box._max.y <= self._prev_position.y {
        ...
        if !self._is_falling && (move_delta.x != 0.0 || move_delta.z != 0.0) {
            ...
            let point: Vector3<f32> =
                self._position + move_direction * (current_actor_collision._bounding_box._mag_xz + CHECK_DIST);
    ...
    pub fn load_controller_save_data(&mut self, controller_save_data: &CharacterControllerSaveData) {
        ...
        self._running_multiplier = 1f32.max(controller_save_data._running_multiplier);
    }
}
