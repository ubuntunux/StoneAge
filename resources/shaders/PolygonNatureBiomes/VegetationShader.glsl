...
FRAGMENT_SHADER_MAIN()
{
    ...
    out_base_color = mix(texture(_TunkTex, in_vertex_output._texCoord), texture(_LeafTex, in_vertex_output._texCoord), blend_factor);
    ...
    out_tangent_normal = mix(texture(_TrunkNormalMap, in_vertex_output._texCoord).xyz * 2.0 - 1.0, texture(_LeafNormalMap, in_vertex_output._texCoord).xyz * 2.0 - 1.0, blend_factor);
}
#endif